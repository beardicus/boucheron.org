...
def walk_resources_tagged_with(node, tag):
    ...
    for resource in walker():
        try:
            taglist = set(map(defaultfilters.slugify, attrgetter("meta.tags")(resource)))
        except AttributeError:
            continue
        if tags <= taglist:
            yield resource
...
class TaggerPlugin(Plugin):
    ...
    def _process_tags_in_resource(self, resource, tags):
        ...
        for tagname in taglist:
            tagname = str(defaultfilters.slugify(tagname));
            if not tagname in tags:
                tag = Tag(tagname)
                tags[tagname] = tag
                tag.resources.append(resource)
                add_method(Node,
                    'walk_resources_tagged_with_%s' % re.sub(r"-", "_", tagname),
...
{%% set walker = source['walk_resources_tagged_with_%(cleantag)s'] %%}
{%% extends "%(template)s" %%}
"""
