...
const build = require("./generate-atom").build;
...
module.exports = (options) => {
  ...
  return function (files, metalsmith, done) {
    ...
    if (!metadata.collections)
      throw new Error(
        "there are no collections configured - see metalsmith-collections plugin."
      );
    ...
    files[destination] = { contents: Buffer.from(atom, "utf-8") };
    done();
  };
};
